...
int main(void) {
     ...
     *
     * 因此初始化时写 0 = LED 灭。*/
    GPIOF_ODR &= ~((1U << 9) | (1U << 10));   /* PF9=0, PF10=0 → 两个 LED 灭 */

    /* ================================================================
     * 主循环：轮询 KEY_UP，根据按键状态切换 LED 闪烁模式
     *
     * 轮询（Polling）vs 中断（Interrupt）：
     *   - 轮询：CPU 不断读取寄存器检查状态，实时性高但浪费 CPU
     *   - 中断：外设主动通知 CPU，CPU 可以做别的事，实时性也很好
     *
     * 本项目用轮询是因为按键交互不需要高精度，用轮询更简单直观。
     * 实际产品中建议用外部中断（EXTI），避免阻塞 CPU。*/
    while (1) {

        /* 读取 KEY_UP（PA0）的电平
         *
         * GPIOA_IDR：Input Data Register，只读。
         * 每一位反映对应引脚的当前电平：
         *   IDR[0] = PA0 的电平（0=低，1=高）
         *
         * 电路回顾：
         *   KEY_UP 松手 → PA0 被 4.7K 上拉到 3.3V → IDR[0] = 1
         *   KEY_UP 按下 → PA0 被按键短路到 GND      → IDR[0] = 0
         *
         * volatile 的必要性：
         *   - key_up 是 volatile，因为它是"随时可能变化的值"
         *   - 编译器优化可能把 if (key_up) 优化成 if (一次读的值)，
         *     导致循环里的判断永远是第一次读的值，程序卡死。*/
        volatile uint32_t key_up = (GPIOA_IDR & (1U << 0));

        if (key_up) {
            /* ========== 按键按下（KEY_UP = 1，即 PA0=高电平）==========
             *
             * 注意：这里 key_up=1 表示"松手"，不是"按下"！
             * 因为 PA0 通过 4.7K 上拉电阻被拉高。
             *
             * 交替闪烁：红 → 绿 → 红 → 绿 ...
             * 原理：用 ^=（异或）翻转 ODR 的某一位来切换 LED 状态
             *
             * 异或翻转原理：
             *   ODR ^= (1 << 9)   → 如果 ODR[9]=0 则变 1，如果 ODR[9]=1 则变 0
             *   即：每执行一次，PF9 的电平翻转一次（toggle）*/

            /* PF9(RED) 亮 200ms */
            GPIOF_ODR ^= (1U << 9);       /* 异或翻转 PF9：灭→亮 或 亮→灭 */
            delay_ms(200);
            GPIOF_ODR &= ~(1U << 9);     /* 手动关闭 PF9，准备切到 GREEN */

            /* PF10(GREEN) 亮 200ms */
            GPIOF_ODR ^= (1U << 10);      /* 异或翻转 PF10 */
            delay_ms(200);
            GPIOF_ODR &= ~(1U << 10);    /* 关闭 GREEN */

        } else {
            /* ========== 按键松开（KEY_UP = 0，即 PA0=低电平）==========
             *
             * 两个 LED 同时闪烁（同步模式）：
             *   → 亮：PF9=1 且 PF10=1
             *   → 灭：PF9=0 且 PF10=0
             *
             * ^= 操作同时翻转两个 LED，
             * 保证它们始终处于相同状态（同时亮或同时灭）。*/

            /* 同时翻转 PF9 和 PF10 */
            GPIOF_ODR ^= (1U << 9) | (1U << 10);   /* 两个 LED 一起 toggle */
            delay_ms(300);
        }
    }
}
